...
export async function POST(request: NextRequest) {
  ...
  // TODO: You have to add validation here to check both passwords are sameconst submitForm = async () => {
    if (payload.password !== payload.password_confirmation) {
      return NextResponse.json({
        status: 400,
        message: "Passwords do not match. Please ensure the password and confirm password are the same.",
      });
    }
